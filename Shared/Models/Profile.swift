...
final class UserProfile {
    ...
    var createdAt: Date
    /// Optional addition allows existing stores to migrate without discarding history.
    var muscleNotesJSON: String?
    ...
    func muscleNote(for muscle: MuscleGroup) -> String {
        muscleNote(forKey: muscle.rawValue)
    }

    func muscleNote(forKey key: String) -> String {
        guard let data = muscleNotesJSON?.data(using: .utf8),
              let notes = try? JSONDecoder().decode([String: String].self, from: data) else { return "" }
        return notes[key] ?? ""
    }

    func setMuscleNote(_ note: String, for muscle: MuscleGroup) throws {
        try setMuscleNote(note, forKey: muscle.rawValue)
    }

    func setMuscleNote(_ note: String, forKey key: String) throws {
        var notes: [String: String] = [:]
        if let data = muscleNotesJSON?.data(using: .utf8) {
            notes = try JSONDecoder().decode([String: String].self, from: data)
        }
        notes[key] = note.isEmpty ? nil : note
        muscleNotesJSON = String(data: try JSONEncoder().encode(notes), encoding: .utf8)
    }
}
